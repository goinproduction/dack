...
export default class ProductService {
  ...
  getCategoryById = (id: string, _page: number, _limit: number) => {
    return new Promise<any>(async (resolve, reject) => {
      try {
        const data = await Category.findOne({ _id: id }).populate({
          path: "productLst",
        });
        if (data) {
          // // pagnination
  ...
  searchProductByTitle = (payload: string) => {
    return new Promise<any>(async (resolve, reject) => {
      try {
        let productLst = await Product.find({ $text: { $search: payload } })
          .limit(3)
          .exec();
  ...
  updateOrderStatus = (id: string, orderStatus: string) => {
    return new Promise<any>(async (resolve, reject) => {
      try {
        ...
        const data = await Order.findOneAndUpdate({ _id: id }, update, {
          new: true,
        })
          .populate({ path: "userId" })
          .populate("details.product")
  ...
  getOrderById = (orderId: string) => {
    return new Promise<any>(async (resolve, reject) => {
      try {
        const data = await Order.find({ _id: orderId })
          .populate("userId")
          .populate("details.product")
          .exec();
        if (data) {
