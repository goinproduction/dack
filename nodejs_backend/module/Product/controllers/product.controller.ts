...
export default class ProductController {
  ...
  handleCreateProduct = async (
    ...
    try {
      const object = await this.ProductService.createProduct(
        data.name,
        data.salePrice,
        data.regularPrice,
        data.description,
      ...
      if (object.success) {
        if (object.page !== null) {
          return res.status(200).json({
            ...
            message: object.message,
            page: object.page,
            productLst: object.response.map((obj: any) =>
              serializeGetProduct(obj)
            ),
          });
        }
        return res.status(200).json({
  ...
  handleCreateCategory = async (
    ...
    try {
      const object = await this.ProductService.createCategory(
        data.title,
        data.image,
        data.productLst
      );
  ...
  handleGetCategoryById = async (
    ...
    try {
      ...
      console.log(_page, _limit);
      const object = await this.ProductService.getCategoryById(
        categoryId,
        _page,
        _limit
      );
  ...
  handleSearchProductByTitle = async (
    req: Request,
    res: Response
  ): Promise<Response> => {
    try {
  ...
  handleCheckout = async (req: Request, res: Response): Promise<Response> => {
    try {
      ...
      const object = await this.ProductService.checkout(
        data.userId,
        data.details,
        data.total_price,
        data.payment_status,
  ...
  handleUpdatePaymentStatus = async (
    req: Request,
    res: Response
  ): Promise<Response> => {
    try {
  ...
  handleUpdateOrderStatus = async (
    ...
    try {
      ...
      const object = await this.ProductService.updateOrderStatus(
        id,
        orderStatus
      );
      if (object.success) {
