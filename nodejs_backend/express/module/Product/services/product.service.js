...
class ProductService {
    constructor() {
        ...
        this.getCategoryById = (id, _page, _limit) => {
            return new Promise(async (resolve, reject) => {
                try {
                    const data = await product_model_1.Category.findOne({ _id: id }).populate({
                        path: "productLst",
                    });
                    if (data) {
                        const response = {
        ...
        this.searchProductByTitle = (payload) => {
            return new Promise(async (resolve, reject) => {
                try {
                    let productLst = await product_model_1.Product.find({ $text: { $search: payload } })
                        .limit(3)
                        .exec();
        ...
        this.updateOrderStatus = (id, orderStatus) => {
            return new Promise(async (resolve, reject) => {
                try {
                    ...
                    const data = await product_model_1.Order.findOneAndUpdate({ _id: id }, update, {
                        new: true,
                    })
                        .populate({ path: "userId" })
                        .populate("details.product")
        ...
        this.getOrderById = (orderId) => {
            return new Promise(async (resolve, reject) => {
                try {
                    const data = await product_model_1.Order.find({ _id: orderId })
                        .populate("userId")
                        .populate("details.product")
                        .exec();
                    if (data) {
